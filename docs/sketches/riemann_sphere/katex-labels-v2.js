...
function makeLabel(htmlParent, frostedBG=false, fontSize=undefined){
    /*
    Need this CSS:
    .katexlabel{
        position: absolute;
...
*/
    ...
    htmlParent.appendChild(positioningElem);
...
function screenSpaceCoordsOf3DPoint(point, camera, renderer){
    ...
    let vec = spareVec3.applyMatrix4(camera.matrixWorldInverse).applyMatrix4(camera.projectionMatrix);
    let arr = [vec.x, vec.y];
    ...
    renderer.getSize(spareVec2); //might not take into account CSS related transforms
...
class Dynamic3DText{
    ...
    constructor(options){
        ...
        camera: a THREE.Camera used to project from
        renderer: the THREE.Renderer this is rendering to. used to get the width and height of canvas
        htmlParent: a HTML element that this text's HTML will be a child of.
        */

        this.camera = options.camera; //required
        this.renderer = options.renderer; //required.
        this.htmlParent = options.htmlParent;
        ...
        this.htmlElem = makeLabel(this.htmlParent, this.frostedBG, this.textSize);
    ...
    updatePositionIfNeeded(t){
        ...
        this.position2D = screenSpaceCoordsOf3DPoint(this._position3D, this.camera, this.renderer);
