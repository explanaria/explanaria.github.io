...
//no #version 300 es in the shader headers because three.js inserts some boilerplate above it
...
void main(){
//  //resultingColor = vec4(vNormal.xyz, 1.0); // view debug normals
//  //if(vNormal.x < 0.0){resultingColor = vec4(offSpecular(color.rgb), 1.0);}else{gl_FragColor = vec4((color.rgb), 1.0);} //view specular and non-specular colors
//  resultingColor = vec4(mod(vUv.xy,1.0),0.0,1.0); //show uvs
  ...
  resultingColor = colorWithGridlines;	
}`;
